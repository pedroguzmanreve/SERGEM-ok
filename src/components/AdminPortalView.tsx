...
export const AdminPortalView: React.FC<AdminPortalViewProps> = ({
  ...
  // Available Jefes de Zona / Supervisores para asignación
  const jefesDeZona = employees.filter(
    (e) =>
      e.rol === 'Jefe de Zona' ||
      e.rol === 'Jefe de Operaciones' ||
      e.rol === 'Jefe Inmediato' ||
      e.rol === 'Coordinador'
  );
  ...
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    const newEmp: Employee = {
      ...
      jefeZonaId: (rol === 'Repartidor' || rol === 'Auxiliar') ? (jefeZonaId || undefined) : undefined,
      email,
    ...
    <div id="admin-portal-view" className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
        {[
          ...
          {
            role: 'Jefe Inmediato' as UserRole,
            title: 'Jefe Inmediato',
            count: employees.filter((e) => e.rol === 'Jefe Inmediato').length,
          ...
          },
          {
            ...
            title: 'Coordinador',
            count: employees.filter((e) => e.rol === 'Coordinador').length,
          ...
          },
          {
            ...
            title: 'Analista',
            count: employees.filter((e) => e.rol === 'Analista').length,
          ...
          },
          {
            ...
            title: 'Auxiliar',
            count: employees.filter((e) => e.rol === 'Auxiliar').length,
            badgeBg: 'bg-orange-100 text-orange-800 border-orange-200',
            borderAccent: 'border-l-4 border-l-orange-600',
            icon: UserCog,
          ...
          return (
            <div
              key={stat.role}
              onClick={() => setRoleFilter(roleFilter === stat.role ? 'TODOS' : stat.role)}
              className={`bg-white rounded-xl p-3 shadow-xs border border-slate-200 hover:shadow-sm transition-all flex flex-col justify-between cursor-pointer ${stat.borderAccent} ${
                roleFilter === stat.role ? 'ring-2 ring-slate-900 bg-slate-50' : ''
              }`}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 truncate" title={stat.title}>
                  {stat.title}
                </span>
                <div className={`p-1 rounded-md ${stat.badgeBg} border shadow-2xs`}>
                  <IconComponent className={`w-3 h-3 ${stat.iconColor}`} />
                </div>
              </div>
              ...
              <div className="flex items-baseline space-x-1 mt-0.5">
                ...
                <span className="text-[10px] text-slate-400 font-medium">colab.</span>
              </div>
            </div>
          );
        })}
              ...
              {filteredEmployees.map((emp) => {
                ...
                return (
                  <tr key={emp.id} className="hover:bg-slate-50/90 transition-colors">
                    ...
                    {/* Jefe de Zona / Inmediato */}
                    <td className="py-3.5 px-4">
                      {emp.rol === 'Repartidor' || emp.rol === 'Auxiliar' ? (
                        jefeAsignado ? (
                        ...
                        <span className="text-slate-400 italic text-[11px]">N/A (Liderazgo)</span>
                      )}
                    </td>
          ...
          >
            ...
            {inviteTab === 'manual' && (
              <form onSubmit={handleInviteSubmit} className="p-6 space-y-4 text-xs overflow-y-auto flex-1">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setRol(newRol);
                        if (newRol === 'Jefe Inmediato') {
                          setCargo('Jefe Inmediato de Operaciones');
                        } else if (newRol === 'Coordinador') {
                          setCargo('Coordinador de Operaciones / Logística');
                        } else if (newRol === 'Analista') {
                          setCargo('Analista de Operaciones / Nómina');
                        } else if (newRol === 'Auxiliar') {
                          setCargo('Auxiliar Operativo / Administrativo');
                        } else if (newRol === 'Repartidor') {
                          setCargo('Repartidor Motorizado');
                        } else if (newRol === 'Jefe de Zona') {
                          setCargo('Jefe de Zona Operacional');
                        } else if (newRol === 'Jefe de Operaciones') {
                          setCargo('Jefe de Operaciones Nacional');
                        } else if (newRol === 'Administrativo') {
                          setCargo('Gestión Administrativa');
                        }
                      }}
                ...
                {/* Vehicle plate (Optional) & Jefe Inmediato / Zona assignment for Repartidor / Auxiliar */}
                {(rol === 'Repartidor' || rol === 'Auxiliar') && (
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                    <span className="text-xs font-bold text-slate-800 block">
                      {rol === 'Repartidor' ? 'Configuración de Operación para Repartidores:' : 'Configuración de Asignación y Operación:'}
                    </span>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="block font-bold text-slate-700 mb-1">
                          Asignar Jefe Inmediato / Supervisor
                        </label>
                        <select
                          value={jefeZonaId}
                        ...
                        >
                          {jefesDeZona.length === 0 ? (
                            <option value="">(Sin supervisores o jefes creados aún - asignar después)</option>
                          ) : (
                            <>
                              <option value="">-- Seleccionar Jefe / Supervisor --</option>
                              {jefesDeZona.map((jefe) => (
                                <option key={jefe.id} value={jefe.id}>
                                  {jefe.nombre} {jefe.apellido} ({jefe.rol} - {jefe.cargo})
                                </option>
                              ))}
                            </>
                          )}
